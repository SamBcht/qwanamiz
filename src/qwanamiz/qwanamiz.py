...
def find_neighbors(complete_df, edges, graph):

    # Initialize the neighbor column
    if not 'neighbors' in complete_df.columns:
    ...
    for edge in edges:
        ...
        neighbors = [tuple(sorted([edge[0], x])) for x in graph[edge[0]]]
        neighbors += [tuple(sorted([edge[1], x])) for x in graph[edge[1]]]
